...
/*
TEST_CASE("Participant: sendsStreamWithMid", "[participant]") {

    Session s{0, test::sfuConfig};
    auto id = s.addParticipant("127.0.0.1", 23292);
    auto participant = s.getParticipant(id);

    SECTION("returns false for a non-existing stream") {
        CHECK_FALSE(participant.sendsStreamWithMid("0"));
    }

    SECTION("returns true for an existing stream") {
        SDP::SessionDescription sdp(test::sdpOfferLines1);
        participant.addOutgoingStream(Stream{sdp.mediaDescriptions[0]});
        CHECK(participant.sendsStreamWithMid("0"));
    }
}
*/
/*
TEST_CASE("Participant: receivesStreamWithMid", "[participant]") {
    ...
    SECTION("returns true for an existing stream") {
        SDP::SessionDescription sdp(test::sdpOfferLines1);
        participant.addIncomingStream(Stream{sdp.mediaDescriptions[0]});
        CHECK(participant.receivesStreamWithMid("0"));
    }
}
*/